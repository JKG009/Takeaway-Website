...
export default function MainPage() {
  ...
  const toggleDisplayDetailsOnClick = (i) => {
    ...
    setTimeout(() => {
      displayDetail === true && i !== articleDisplayed
        ? setArticleDisplayed(i)
        : setDisplayDetail(!displayDetail);
    }, 100);
  };
  ...
  return (
    <Main>
      <Main.TitleContainer>
        <Main.Title>
          Golden Wok<span style={{ display: "block" }}>Chinese Takeaway</span>
        </Main.Title>
        <Main.Subtitle>
      ...
      <Main.TextContainer display="grid">
        <Main.Text style={{ borderBottom: "#1C2826 solid 1px" }}>
          Welcome to the Golden Wok, here you will find details about us, our
          food, contact & location and any questions that you might have before
          ordering from us.
        </Main.Text>
        <Main.Text>
          If you would like to try something new, below are some of our most
          popular dishes that our customers always order.{" "}
          <span style={{ color: "#DF9216" }}>
            Click on the images to get more details.
          </span>
        </Main.Text>
        <Main.Text gridColumnEnd={"span 2"}>
          Click{" "}
          <Navbar.Anchor href={menu} target="_blank" style={{ padding: "0" }}>
            here
          </Navbar.Anchor>{" "}
      ...
      {isMediumDevice ? (
        <Main.GridContainer>
          {articles.map((article, index) => (
            <Main.Article
              key={index}
              onClick={() => toggleDisplayDetailsOnClick(index)}
              displayDetail={displayDetail}
              backgroundImg={article.background}
            >
